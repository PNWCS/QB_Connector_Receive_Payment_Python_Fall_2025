...
def get_txn_id_from_payment_term():
    inv_query = (
        '<?xml version="1.0" encoding="utf-8"?>'
        '<?qbxml version="16.0"?>'
        "<QBXML>"
    ...
    try:
        ...
        raise RuntimeError(
            f"Local XML error (InvoiceQueryRq): {e}\n---\n{inv_query}\n---"
        )
...
def key_term(t):
    return (t.customer, t.invoice_number, t.child_id, t.date, round(float(t.amount), 2))


def add_payment_term(
    company_file: str | None, payments: List[CustomerReceivePaymentTerms]
) -> List[CustomerReceivePaymentTerms]:
    rp_add_reqs = []
    root = get_txn_id_from_payment_term()
    invoices = []
    for inv in root.findall(".//InvoiceRet"):
        ref = (inv.findtext("RefNumber") or "").strip()
        txnid = (inv.findtext("TxnID") or "").strip()
        invoices.append((ref, txnid))
    for payment in payments:
        customer = payment.customer
        inv_ref = payment.invoice_number
        if not customer:
            raise ValueError("payment.customer is required")
        if not inv_ref:
            raise ValueError("payment.invoice_number is required")

        date_s = _qb_date(payment.date)
        memo_src = payment.child_id
        memo_x = _esc(memo_src)
        customer_x = _esc(customer)
        invoice_map = {str(ref).strip(): str(txn).strip() for ref, txn in invoices}
        ref = str(payment.invoice_number).strip()
        txnid = invoice_map.get(ref)

        total_amount_s = _qb_amount(payment.amount)  # returns "0.00" string
        ...
        if not applied_xml:
            raise RuntimeError(
                f"No invoice TxnID(s) found for invoice reference: {inv_ref}"
            )
        applied_block = "\n".join(applied_xml)
    ...
    try:
        ...
        raise RuntimeError(
            f"Local XML error (ReceivePaymentAddRq): {e}\n---\n{batch_rp_add}\n---"
        )

    # Send and parse response
    _send_qbxml(batch_rp_add)
    test = fetch_payment_terms()
    test_keys = {key_term(t) for t in test}
    present = [x for x in payments if key_term(x) in test_keys]
    # missing = [x for x in payments if key_term(x) not in test_keys]
    return present
